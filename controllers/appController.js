...
exports.getIndexPage = (req, res) => {
  ...
  res.json({ status: 200, message: 'Broker-Web-Service CRUD RESTFUL API using Node JS, Express JS, and Redis.' });
};
...
exports.addUser = (req, res) => {
  ...
  redisClient.exists(newUser.id, (err, reply) => {
    if (reply === 1) {
...
exports.deleteUser = (req, res) => {
  const { userId } = req.params;
  redisClient.del(userId, (err, result) => {
    if (err) {
      return res.json({ status: 400, message: 'Something went wrong', err });
    }
